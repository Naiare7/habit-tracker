...
- [x] Crear `StatsDAO.java` en el paquete `dao` con queries SQL que calculan:
  - [x] `getWeeklyStats(int userId)` — cuántos hábitos se completaron cada día de la semana actual
  - [x] `getMonthlyStats(int userId, int year, int month)` — porcentaje de cumplimiento del mes
  - [x] `getStreak(int habitId)` — cuántos días consecutivos se ha completado el hábito
- [x] Usar `PreparedStatement` con las fechas calculadas en Java antes de pasarlas a la query
- [x] Crear `StatsService.java` que usa `StatsDAO`
- [x] Comentar cada método

#### 5.2 Crear el Servlet de estadísticas ✅

- [x] Crear `StatsServlet.java` mapeado a `/api/stats`
- [x] `doGet` con diferentes sub-rutas:
  - [x] `/api/stats/weekly` — estadísticas semanales
  - [x] `/api/stats/monthly?year=YYYY&month=MM` — estadísticas mensuales
  - [x] `/api/stats/streak?habitId=X` — racha de un hábito
- [x] Devolver el resultado como JSON con Gson
- [x] Comentar el Servlet

#### 5.3 Crear el modelo de estadísticas en Angular ✅

- [x] Crear la interfaz `WeeklyStats` y `MonthlyStats` en `models/stats.model.ts`
- [x] Crear `StatsService` en `services/stats.service.ts` con los métodos que llaman a cada endpoint

#### 5.4 Crear el gráfico de barras semanal ✅

- [x] Crear el componente `components/stats/weekly-chart`
- [x] Usar Chart.js (ng2-charts) para mostrar un gráfico de barras
- [x] El eje X son los días de la semana, el eje Y el número de hábitos completados
- [x] Usar los colores de la paleta cozy definida en la guía de estilo

#### 5.5 Crear el gráfico circular mensual ✅

- [x] Crear el componente `components/stats/monthly-chart`
- [x] Mostrar un gráfico de tipo doughnut con el porcentaje de días del mes con todos los hábitos completados
- [x] Añadir el número grande en el centro del gráfico (porcentaje)

#### 5.6 Crear el calendario de hábitos ✅

- [x] Crear el componente `components/stats/habit-calendar`
- [x] Mostrar una cuadrícula con los días del mes
- [x] Colorear cada día según el nivel de cumplimiento: gris (sin datos), rojo suave (0%), amarillo (50%), verde (100%)
- [x] Al pasar el ratón por encima de un día, mostrar un tooltip con los detalles

#### 5.7 Crear la pantalla de estadísticas ✅

- [x] Crear el componente `pages/stats`
- [x] Componer los tres componentes anteriores en una sola pantalla
- [x] Añadir un selector para cambiar entre semana/mes
- [x] Mostrar la racha actual del hábito más largo
